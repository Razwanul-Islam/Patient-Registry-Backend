...
class RegistryFields(serializers.ModelSerializer):
    ...
    def create(self,data):
        if data.get('id'):
            patientField = FieldOfRecord.objects.get(id=int(data.get('id')),patient_record=data.get('patient_record'))
            patientField.name=data.get('name')
            patientField.type=data.get('type')
        else:
            patientField = FieldOfRecord(patient_record=data.get('patient_record'),name=data.get('name'),type=data.get('type'))
...
class PatientRecordSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = PatientRecord
        fields = ('id','fieldofrecord_set','created_at','updated_at')
    def create(self):
        registry_fields_data = self.data.pop('fieldofrecord_set')
        print(self.data.get('id'))
        if self.data.get('id'):
            print(self.data.get('id'))
            PatientRecordObject = PatientRecord.objects.get(id=int(self.data.get('id')),user=self.context.get('user'))
        else:
            PatientRecordObject = PatientRecord.objects.create(user=self.context.get('user'))
        ...
        registryFields = registry_fields_serializers.create(registry_fields_data)
